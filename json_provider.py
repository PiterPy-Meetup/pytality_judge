...
class JSONProvider(faker.providers.BaseProvider):
    ...
    def _find_refeence_in_schema(path, schema):
        ...
        if len(path_elements) == 1:
            ...
            return JSONProvider._find_refeence_in_schema(
                "/".join(path_elements[1:]), cschema
            )

    @staticmethod
    def value_for_schema_element(
        schema_element, root_schema_element, fake=faker.Faker(), overrides=dict()
    ):
        ...
        if element_type == "null":
            ...
            return round(
                random.uniform(
                    se.get("minimum", -100000000), se.get("maximum", 10000000)
                ),
                random.randint(0, 5),
            )
            ...
            for _ in range(
                0,
                random.randint(
                    0, random.randint(se.get("minItems", 3), se.get("maxItems", 100))
                ),
            ):
                array_value.append(
            ...
            raise ValueError(
                "Don't know have to create value for schema element type [{}]".format(
                    se["type"]
                )
            )
